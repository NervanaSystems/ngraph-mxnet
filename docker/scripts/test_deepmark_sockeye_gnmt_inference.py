...
#   $ pip -U pytest
#   $ pytest test_deepmark_sockeye_gnmt.py
#   $ deactivte
#
...
# For example:
#
#     $ pytest -s ./test_sockeye_gnmt.py --junit-xml=../validation_deepmark_sockeye_gnmt.xml --junit-prefix=daily_validation_deepmark_sockeye_gnmt
...
def test_deepmark_sockeye_gnmt_cpu_backend():
    ...
    ngraphLog = VT.runSockeyeGNMTDeepMarkScript(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    if (not checkAccurary):
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_sockeye_gnmt_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_sockeye_gnmt_cpu_ngraph.log'))
    else:
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_sockeye_gnmt_accuracy_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_sockeye_gnmt_accuracy_cpu_ngraph.log'))
        assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_sockeye_gnmt_accuracy_cpu_ngraph.log')) == True

    #writeJenkinsDescription(ngraphLog, os.path.join(lDir, 'test_deepmark_sockeye_gnmt_jenkins_oneline.log'))


    print("----- Deepmark sockeye_GNMT Testing Summary ----------------------------------------")

    summaryLog = None
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_sockeye_gnmt_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_sockeye_gnmt_cpu_summary.log')
...
# End: test_deepmark_sockeye_gnmt_cpu_backend()
