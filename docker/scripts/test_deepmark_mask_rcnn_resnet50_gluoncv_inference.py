...
# For example:
#
#     $ pytest -s ./test_deepmark_mask_rcnn_resnet50_gluoncv.py --junit-xml=../validation_deepmark_mask_rcnn_resnet50_gluoncv.xml --junit-prefix=daily_validation_deepmark_mask_rcnn_resnet50_gluoncv
...
def test_deepmark_mask_rcnn_resnet50_gluoncv_cpu_backend():
    ...
    ngraphLog = VT.runMASKRCNNGLUONCVDeepMarkScript(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    if (not checkAccurary):
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_mask_rcnn_resnet50_gluoncv_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_mask_rcnn_resnet50_gluoncv_cpu_ngraph.log'))
    else:
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_mask_rcnn_resnet50_gluoncv_accuracy_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_mask_rcnn_resnet50_gluoncv_accuracy_cpu_ngraph.log'))
        assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_mask_rcnn_resnet50_gluoncv_accuracy_cpu_ngraph.log')) == True

    #writeJenkinsDescription(ngraphLog, os.path.join(lDir, 'test_deepmark_a3c_jenkins_oneline.log'))


    print("----- Deepmark A3C Testing Summary ----------------------------------------")

    summaryLog = None
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_mask_rcnn_resnet50_gluoncv_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_mask_rcnn_resnet50_gluoncv_cpu_summary.log')
...
# End: test_deepmark_mask_rcnn_resnet50_gluoncv_cpu_backend()
