...
# For example:
#
#     $ pytest -s ./test_deepmark_squeezenet_inference.py --junit-xml=../validation_deepmark_squeezenet.xml --junit-prefix=daily_validation_deepmark_squeezenet
...
def test_deepmark_squeezenet_cpu_backend():
    ...
    ngraphLog = VT.runSqueezenetDeepMarkScript(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    if (not checkAccurary):
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_squeezenet_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_squeezenet_cpu_ngraph.log'))
        print("\n----- Deepmark Squeezenet1.1 Inference Performance Testing Summary ------\n")
    else:
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_squeezenet_accuracy_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_squeezenet_accuracy_cpu_ngraph.log'))
        assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_squeezenet_accuracy_cpu_ngraph.log')) == True
        print("\n----- Deepmark Squeezenet1.1 Inference Accuracy Testing Summary ------\n")

    #writeJenkinsDescription(ngraphLog, os.path.join(lDir, 'test_deepmark_squeezenet_jenkins_oneline.log'))


    print("----- Deepmark squeezenet1.1 Testing Summary ----------------------------------------")

    summaryLog = None
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_squeezenet_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_squeezenet_cpu_summary.log')
...
# End: test_deepmark_squeezenet_cpu_backend()
