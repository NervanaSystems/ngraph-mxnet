...
# For example:
#
#     $ pytest -s ./test_deepmark_a3c.py --junit-xml=../validation_deepmark_resnet_50.xml --junit-prefix=daily_validation_deepmark_resnet_50
...
def test_deepmark_a3c_cpu_backend():
    ...
    ngraphLog = VT.runA3CDeepMarkScript(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    if (not checkAccurary):
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_a3c_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_a3c_cpu_ngraph.log'))
    else:
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_a3c_accuracy_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_a3c_accuracy_cpu_ngraph.log'))
        assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_a3c_accuracy_cpu_ngraph.log')) == True
    ...
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_a3c_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_a3c_cpu_summary.log')
...
# End: test_deepmark_a3c_cpu_backend()
