...
#   $ pip -U pytest
#   $ pytest test_deepmark_inception_v3.py
#   $ deactivte
#
...
# For example:
#
#     $ pytest -s ./test_deepmark_inception-v3.py --junit-xml=../validation_deepmark_inception_v3_cpu.xml --junit-prefix=daily_validation_inception_v3_cpu
...
def test_deepmark_inception_v3_cpu_backend():
    ...
    ngraphLog = VT.runInceptionV3Script(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    if (not checkAccurary):
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_inception_v3_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_inception_v3_cpu_ngraph.log'))
        print("\n----- Deepmark INCEPTION V3 Inference Perfomance Testing Summary ------\n")
    else:
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_inception_v3_accuracy_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_inception_v3_accuracy_cpu_ngraph.log'))
        assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_inception_v3_accuracy_cpu_ngraph.log')) == True
        print("\n----- Deepmark INCEPTION V3 Inference Accuracy Testing Summary ------\n")

    summaryLog = None
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_inception_v3_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_inception_v3_cpu_summary.log')
...
# End: test_deepmark_inception_v3_cpu_backend()
