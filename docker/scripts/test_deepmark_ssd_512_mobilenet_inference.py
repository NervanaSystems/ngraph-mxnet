...
# For example:
#
#     $ pytest -s ./test_deepmark_ssd_512_mobilenet.py --junit-xml=../validation_deepmark_ssd_512_mobilenet.xml --junit-prefix=daily_validation_deepmark_ssd_512_mobilenet
...
def test_deepmark_ssd_512_mobilenet_cpu_backend():
    ...
    ngraphLog = VT.runSDDMobileNetDeepMarkScript(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    if (not checkAccurary):
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_ssd_512_mobilenet_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_ssd_512_mobilenet_cpu_ngraph.log'))
    else:
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_ssd_512_mobilenet_accuracy_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_ssd_512_mobilenet_accuracy_cpu_ngraph.log'))
        assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_ssd_512_mobilenet_accuracy_cpu_ngraph.log')) == True
    ...
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_ssd_512_mobilenet_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_ssd_512_mobilenet_cpu_summary.log')
...
# End: test_deepmark_ssd_512_mobilenet_cpu_backend()
