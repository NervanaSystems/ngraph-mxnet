...
# For example:
#
#     $ pytest -s ./test_deepmark_sockeye_transformer.py --junit-xml=../validation_deepmark_sockeye_transformer.xml --junit-prefix=daily_validation_deepmark_sockeye_transformer
...
def test_deepmark_sockeye_transformer_cpu_backend():
    ...
    ngraphLog = VT.runSockeyeTransformerDeepMarkScript(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    if (not checkAccurary):
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_sockeye_transformer_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_sockeye_transformer_cpu_ngraph.log'))
    else:
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_sockeye_transformer_accuracy_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_sockeye_transformer_accuracy_cpu_ngraph.log'))
        assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_sockeye_transformer_accuracy_cpu_ngraph.log')) == True

    #writeJenkinsDescription(ngraphLog, os.path.join(lDir, 'test_deepmark_sockeye_transformer_jenkins_oneline.log'))

    print("----- Deepmark Sockeye_Transformer Testing Summary ----------------------------------------")

    summaryLog = None
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_sockeye_transformer_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_sockeye_transformer_cpu_summary.log')
...
# End: test_deepmark_sockeye_transformer_cpu_backend()
