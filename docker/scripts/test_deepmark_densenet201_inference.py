...
# For example:
#
#     $ pytest -s ./test_deepmark_densenet201_inference.py --junit-xml=../validation_deepmark_densenet201.xml --junit-prefix=daily_validation_deepmark_densenet201
...
def test_deepmark_densenet201_cpu_backend():
    ...
    ngraphLog = VT.runDensenet201DeepMarkScript(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    if (not checkAccurary):
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_densenet201_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_densenet201_cpu_ngraph.log'))
    else:
        VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_densenet201_accuracy_cpu_ngraph.log'))
        VT.checkScript(os.path.join(lDir, 'test_deepmark_densenet201_accuracy_cpu_ngraph.log'))
        assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_densenet201_accuracy_cpu_ngraph.log')) == True


    #writeJenkinsDescription(ngraphLog, os.path.join(lDir, 'test_deepmark_densenet201_jenkins_oneline.log'))


    print("----- Deepmark Densenet201 Testing Summary ----------------------------------------")

    summaryLog = None
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_densenet201_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_densenet201_cpu_summary.log')
...
# End: test_deepmark_densenet201_cpu_backend()
