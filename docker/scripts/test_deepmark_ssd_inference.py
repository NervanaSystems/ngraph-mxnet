...
# For example:
#
#     $ pytest -s ./test_deepmark_ssd.py --junit-xml=../validation_deepmark_ssd.xml --junit-prefix=daily_validation_deepmark_ssd
...
def test_deepmark_ssd_cpu_backend():
    ...
    ngraphLog = VT.runSSDDeepMarkScript(sourceDir=sourceDir,
                                logID=' nGraph',
                                script=script,
    ...
    #else:
    #    VT.writeLogToFile(ngraphLog, os.path.join(lDir, 'test_deepmark_ssd_accuracy_cpu_ngraph.log'))
    #    VT.checkScript(os.path.join(lDir, 'test_deepmark_ssd_accuracy_cpu_ngraph.log'))
    #    assert VT.checkAccuracyResult(os.path.join(lDir, 'test_deepmark_ssd_accuracy_cpu_ngraph.log')) == True

    #writeJenkinsDescription(ngraphLog, os.path.join(lDir, 'test_deepmark_ssd_jenkins_oneline.log'))


    print("----- Deepmark SSD Testing Summary ----------------------------------------")

    summaryLog = None
    if lDir != None:
        if checkAccurary:
            summaryLog = os.path.join(lDir, 'test_deepmark_ssd_accuracy_cpu_summary.log')
        else:
            summaryLog = os.path.join(lDir, 'test_deepmark_ssd_cpu_summary.log')
...
# End: test_deepmark_ssd_cpu_backend()
