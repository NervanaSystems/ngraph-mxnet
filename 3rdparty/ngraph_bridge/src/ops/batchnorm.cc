...
static NgraphNodePtr create_batchnorm_basic_computation_nodes(
    ...
    const NgraphNodePtr& ng_in_data, const size_t channel_axis,
  ...
  const ngraph::Shape channel_vector_plus_axes_shape =
      get_vector_plus_axes_shape(batch_data_rank, channel_axis,
                                 channel_axis_length);
  ...
  const NgraphNodePtr ng_epsilon_shaped =
      makeConstant(et, channel_vector_plus_axes_shape, epsilon);
...
create_batchnorm_training_without_ngraph_bn_op(
  ...
  const NgraphNodePtr ng_batch_means = Emitter::ReduceAxes(
      ng_in_data, {channel_axis}, true, true, ngraph::builder::mean);
  ...
  const NgraphNodePtr ng_normalized_batch =
      create_batchnorm_basic_computation_nodes(
          ng_batch_means, ng_batch_variances, ng_in_data, channel_axis, epsilon,
          ng_maybe_gamma, ng_beta);
...
NgraphNodePtr create_batchnorm_inference_without_ngraph_bn_op(
  ...
  const NgraphNodePtr ng_normalized_batch =
      create_batchnorm_basic_computation_nodes(
          ng_moving_mean, ng_moving_var, ng_in_data, channel_axis, epsilon,
          ng_maybe_gamma, ng_beta);
